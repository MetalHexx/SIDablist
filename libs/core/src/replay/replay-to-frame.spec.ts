...
import { replayToFrame } from './replay-to-frame.js';
...
describe('replayToFrame', () => {
  it('lands on the same machine and register state the live path reaches at that frame', () => {
    ...
    const replayed = replayToFrame(file, 1, frames(40), [false, false, false]);
  ...
  it('is deterministic: the same request twice gives the same answer', () => {
    ...
    const first = replayToFrame(file, 1, frames(25), [false, false, false]);
    const second = replayToFrame(file, 1, frames(25), [false, false, false]);
  ...
  it('actually runs the frames rather than stopping at init', () => {
    ...
    const shallow = replayToFrame(file, 1, frames(3), [false, false, false]);
    const deep = replayToFrame(file, 1, frames(30), [false, false, false]);
  ...
  it('runs no frames at all for a target of 0', () => {
    const result = replayToFrame(counterTune(), 1, frames(0), [false, false, false]);
  ...
  it('clamps a negative or fractional target to a whole frame at or after the start', () => {
    expect(replayToFrame(counterTune(), 1, frames(-12), [false, false, false]).frame).toBe(0);
    expect(replayToFrame(counterTune(), 1, frames(7.4), [false, false, false]).frame).toBe(7);
  });

  it('holds a muted voice at 0 throughout, leaving the others live', () => {
    const result = replayToFrame(counterTune(), 1, frames(20), [false, true, false]);

    const [voice0, voice1, voice2] = VOICE_CONTROL_REGISTERS.map(
      (register) => result.registers.values[register],
    );
