...
import type { PlayRate } from './play-rate.js';
 ...
 *  not divide the frame evenly, unlike a rate that could never expose a rounding defect. Latch
 *  `$1FFD`, 8190 cycles per call; 19656 (PAL cycles per frame) / 8190 is exactly 2.4. */
function multispeedMachine(): C64Machine {
  const file = tune([
...
describe('playRateFor', () => {
  it('reports the exact and rounded rates genuinely disagreeing for a multispeed tune', () => {
    const machine = multispeedMachine();
...
describe('playCallIntervalUs', () => {
  it('divides the nominal interval by the rate in force', () => {
    ...
    expect(playCallIntervalUs(PAL_FRAME_INTERVAL_US, exact)).toBeCloseTo(
      PAL_FRAME_INTERVAL_US / 2.4,
      6,
    );
    expect(playCallIntervalUs(PAL_FRAME_INTERVAL_US, rounded)).toBeCloseTo(
      PAL_FRAME_INTERVAL_US / 2,
      6,
    );
  });
...
describe('msToPlayCalls', () => {
  it('converts a real-time duration to a whole number of play calls at the rate in force', () => {
    ...
    expect(msToPlayCalls(milliseconds(1000), PAL_FRAME_INTERVAL_US, unitRate)).toBe(
  ...
  it('scales with the fractional rate rather than the rounded one under exact mode', () => {
    ...
    const exactCalls = msToPlayCalls(milliseconds(1000), PAL_FRAME_INTERVAL_US, exact);
    const roundedCalls = msToPlayCalls(milliseconds(1000), PAL_FRAME_INTERVAL_US, rounded);
  ...
  it('never produces NaN for a non-finite or non-positive nominal interval', () => {
    ...
    for (const bad of [0, -1, Number.NaN]) {
      expect(Number.isFinite(msToPlayCalls(milliseconds(1000), bad, rate))).toBe(true);
    }
  });
});
