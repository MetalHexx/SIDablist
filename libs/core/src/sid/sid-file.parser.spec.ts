...
import { parseSidFile } from './sid-file.parser.js';
import { SidParseError } from './sid-file.model.js';
import { BUNDLED_TUNES, decodeBundledTune } from './__fixtures__/index.js';
...
describe('parseSidFile', () => {
  ...
  it('resolves a v4 thirdSidAddress from the encoded byte', () => {
    const file = parseSidFile(
      buildSidFileBytes({ version: 4, secondSidByte: 0x50, thirdSidByte: 0x42 }),
    );
    expect(file.secondSidAddress).toBe(0xd500);
