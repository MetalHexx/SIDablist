...
import { scanTune, TuneScan } from './scan-tune.js';
...
describe('scanTune', () => {
  ...
  it('fails with a cycle-budget message instead of hanging', () => {
    expect(() => scanTune(runawayTune, 1, 5)).toThrow(
...
describe('TuneScan', () => {
  ...
  it('records nothing past the frame a cycle-budget failure stopped at', () => {
    ...
    expect(() => scan.advanceTo(5)).toThrow('analysis scan exceeded its cycle budget at frame 0');
