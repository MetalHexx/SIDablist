...
import type { LoopDetectOptions } from './loop-detect.js';
...
describe('detectLoop', () => {
  ...
  it('reports the intro length as the loop start for a tune that only repeats after one', () => {
    ...
    const scan = scanOf(1000, (f) => (f < introFrames ? introSeed(f) : (f - introFrames) % 200));
  ...
  describe('option boundaries', () => {
    ...
    it('accepts a tail exactly at minTailFrames, and declines the same stream one frame short', () => {
      ...
      const scan = scanOf(frames, (f) => (f < loopStart ? introSeed(f) : (f - loopStart) % period));
