import { FEATURE_DIMENSION_COUNT } from './frame-features.js';
import type { FeatureMatrix } from './frame-features.js';
import { dimensionWeightsFor, rowDistance } from './novelty.js';
import type { FeatureWeights } from './novelty.js';
...
function computeSectionBoundaries(
  ...
  for (let block = 1; block < blockCount; block++) {
    ...
    if (
      departure[block] >= threshold &&
      departure[block] >= departure[block - 1] &&
      departure[block] >= next
    ) {
